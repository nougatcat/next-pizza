...
const generateProductItem = ({ productId, pizzaType, size }: { productId: number; pizzaType?: 1 | 2; size?: 20 | 30 | 40; }) => {
    return {
        ...
        pizzaType, // тип теста
        size
    } as Prisma.ProductItemUncheckedCreateInput //тип созданный призмой автоматически на основе модели ProductItem
}; //эта функция получает объект с заданными параметрами и возвращает объект с заданными параметрами + size
...
async function up() {
    ...
    const pizza1 = await prisma.product.create({
        data: {
            ...
            categoryId: 1,
            ingredients: {
                connect: _ingredients.slice(0, 5), //какие есть ингредиенты по id
            },
        },
    });
